![Spider-Man GIF by Spider-Man Into The Spider-Verse](https://github.com/user-attachments/assets/e8f90fef-e908-4087-b570-ecd91c960f78)
# 👋 Hi there! I'm Muhammed Rustamov
Java Developer | Aspiring Web Developer | Logic Enthusiast & Basketball Lover🏀

## 🚀 About Me
I’m a passionate beginner Java developer deeply interested in programming fundamentals, algorithms, and crafting logically sound solutions.
Currently diving deep into the Java ecosystem—mastering OOP, Collections, Multithreading, and the Stream API.

## 💡 Beyond Code
🏀 Play basketball and thrive in team environments
🔍 Solve logic puzzles and riddles for fun
📈 Keep up with software development trends and learn something new every day

## ⚙️ Tech Stack & Tools
Category
Skills
Java
Core (OOP, Collections, Multithreading, Stream API)
Web Basics
HTML, CSS (fundamentals)
Tools
Git, Maven

---

## 🌱 What I’m Learning Now
Algorithms & Data Structures in Java
Building Telegram bots
Fundamentals of web application architecture
Principles of RESTful API design

 ---

## 🤝 Open to Collaboration
I’m eager to contribute to open-source projects or join small backend development initiatives to gain hands-on experience and grow as a developer.
If you need a motivated teammate—let’s build something great together!

---

## 📧 Get in Touch
✉️ Email: muhammed56945639@gmail.com

---

## 🔥 Fun Facts About Me
I geek out over logic puzzles and brain teasers
Huge basketball fan—I play on a team and never miss NBA highlights
Constantly leveling up my developer skills and embracing new challenges

---

Thanks for stopping by! 😊  
